// xelis_v4_tape_bench.cu — TapeMix: data-dependent instruction tape
//
// Each hash generates a unique instruction "tape" from its scratchpad.
// Applied every TAPE_FREQ iterations during stage_3.
// CPU: branch predictor learns tape → ~0 overhead after warmup.
// GPU: 32 threads × 32 different tapes → warp diverges every step.
// Branch prediction can't be bitsliced or vectorized.
//
// TAPE_LEN: number of instructions in the tape (8-32)
// TAPE_FREQ: apply tape every N iterations (32-256)
...
#define BASE_SIZE        (531u * 128u)
#define MEMORY_SIZE      (BASE_SIZE * PAD_MULT)
...
// Execute one tape instruction. Opcodes have VARYING cost (key for divergence).
// Light ops: ~2-5 cycles. Heavy ops: ~20-100 cycles.
// With 32 threads having different tapes, warp serializes ALL unique opcodes.
__device__ __noinline__ uint64_t tape_exec(uint32_t opcode, uint64_t s0, uint64_t s1, uint64_t operand) {
    switch (opcode & 0xF) {
        case 0:  return s0 ^ operand;                                    // XOR: 1 cycle
        case 1:  return s0 + operand;                                    // ADD: 1 cycle
        case 2:  return rotl64(s0, operand & 63);                        // ROT: 2 cycles
        case 3:  return s0 * (operand | 1);                              // MUL: 5 cycles
        case 4:  return s0 ^ rotl64(s1, 17) ^ operand;                  // XOR+ROT: 3 cycles
        case 5:  return (s0 + s1) * (operand | 1);                      // ADD+MUL: 6 cycles
        case 6:  return s0 ^ murmurhash3(operand);                       // HASH: ~15 cycles
        case 7:  return rotl64(s0 * s1, operand & 63) ^ operand;        // MUL+ROT: 7 cycles
        case 8:  return s0 ^ (s1 >> (operand & 31));                     // XOR+SHIFT: 2 cycles
        case 9:  return (s0 & operand) | (s1 & ~operand);               // BLEND: 3 cycles
        case 10: return murmurhash3(s0 ^ s1) + operand;                  // HASH2: ~15 cycles
        case 11: return __umul64hi(s0, operand) ^ s1;                    // MULHI: 5 cycles
        case 12: return isqrt_v3(s0 ^ operand);                          // ISQRT: ~30 cycles (HEAVY)
        case 13: { // AES round: ~20 cycles on GPU (HEAVY)
            uint8_t blk[16], k[16];
        ...
        case 14: return modular_power(s0 & 0xFFFF, s1 & 0x1F, operand | 3); // MODPOW: ~50-100 cycles (HEAVIEST)
        case 15: return (uint64_t)(combine_u64(s0, s1) % (operand | 1));     // DIV128: ~30 cycles (HEAVY)
        default: return s0;
    }
}

__global__ void xelis_stage3_tape(uint64_t *all_scratch, uint32_t batch_size)
{
    ...
    // Generate per-hash instruction tape from scratchpad
    // Each hash gets a DIFFERENT tape → threads in warp diverge
    ...
    uint32_t r = 0;
    uint64_t tape_state = addr_a ^ addr_b;  // persistent tape state
    ...
    for (uint32_t i = 0; i < SCRATCHPAD_ITERS; i++) {
        ...
        for (uint32_t j = 0; j < BUFFER_SIZE; j++) {
            ...
            // ====== TAPE APPLICATION (every TAPE_FREQ iterations) ======
            if ((j & (TAPE_FREQ - 1)) == 0) {
                // Execute the per-hash instruction tape.
                // CPU: branch predictor knows this tape → ~2 cycles/instruction.
                // GPU: 32 threads × different tapes → diverge on every step.
                #pragma unroll 1  // prevent unroll to keep divergence realistic
                for (int t = 0; t < TAPE_LEN; t++) {
                    tape_state = tape_exec(tape[t], tape_state, result ^ a, c ^ b);
                }
                // Mix tape output into computation
                result ^= tape_state;
            }
    ...
    mem_buffer_a[0] = addr_a ^ addr_b ^ tape_state;
}

// Also keep V3 baseline for comparison
__global__ void xelis_stage3_v3(uint64_t *all_scratch, uint32_t batch_size)
{
    uint32_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= batch_size) return;
    uint64_t *a = all_scratch + (uint64_t)tid * MEMORY_SIZE;
    uint64_t *b = a + BUFFER_SIZE;
    const uint8_t key[16] = {'x','e','l','i','s','h','a','s','h','-','p','o','w','-','v','3'};
    uint64_t aa = b[BUFFER_SIZE-1], ab = a[BUFFER_SIZE-1] >> 32;
    uint32_t r = 0;
    for (uint32_t i = 0; i < SCRATCHPAD_ITERS; i++) {
        uint64_t ma = a[map_index(aa)], mb = b[map_index(ma^ab)];
        ...
        uint64_t res = ~(h1^h2);
        for (uint32_t j = 0; j < BUFFER_SIZE; j++) {
            uint64_t la=a[map_index(res)],lb=b[map_index(la^~rotr64(res,r))];
            uint64_t c=(r<BUFFER_SIZE)?a[r]:b[r-BUFFER_SIZE]; r=(r<MEMORY_SIZE-1)?r+1:0;
...
int main(int argc, char **argv) {
    ...
    size_t pad_bytes = (size_t)MEMORY_SIZE * 8;

    printf("TapeMix Benchmark — PAD=%d TAPE_LEN=%d TAPE_FREQ=%d\n", PAD_MULT, TAPE_LEN, TAPE_FREQ);
    printf("=====================================================\n");
    ...
    if (batch == 0) {
        ...
        batch = (int)((free_mem * 0.45) / pad_bytes);
        batch = (batch / 128) * 128;
        if (batch < 128) batch = 128;
    }

    size_t scratch_bytes = (size_t)batch * pad_bytes;
    printf("Batch: %d, Warps/SM: %.1f\n", batch, (float)batch/prop.multiProcessorCount/32.0);
    printf("Tape applications per hash: %u\n\n", SCRATCHPAD_ITERS * BUFFER_SIZE / TAPE_FREQ);
    ...
    int tpb=128, grid=(batch+tpb-1)/tpb;

    // Warmup + V3 baseline
    xelis_stage3_v3<<<grid,tpb>>>(d_scratch,batch);
    CUDA_CHECK(cudaDeviceSynchronize());
    xelis_stage3_tape<<<grid,tpb>>>(d_scratch,batch);
    ...
    tape_avg /= (runs > 1 ? runs-1 : 1);

    float v3_khs = batch/(v3_avg/1000.0f)/1000.0f;
    float tape_khs = batch/(tape_avg/1000.0f)/1000.0f;

    printf("RESULTS: %s %dSMs | V3=%.2f kH/s | Tape=%.2f kH/s | %.2fx slower | SM: V3=%.1f Tape=%.1f\n\n",
           prop.name, prop.multiProcessorCount,
           v3_khs, tape_khs, v3_avg/tape_avg < 1 ? tape_avg/v3_avg : v3_avg/tape_avg,
           v3_khs*1000/prop.multiProcessorCount,
           tape_khs*1000/prop.multiProcessorCount);
