// xelis_v4_bigpad_bench.cu — V3 with scaled scratchpad sizes
//
// Strategy: Increase per-thread scratchpad until VRAM limits occupancy.
// No S-box, no cascade, no bypass risk. VRAM is physics, not implementation.
//
// Build: nvcc -O3 -arch=sm_120 -DPAD_MULT=N -o bench_bigpad xelis_v4_bigpad_bench.cu --use_fast_math
// PAD_MULT: 1=544KB, 2=1.1MB, 4=2.1MB, 8=4.2MB, 16=8.5MB
...
// Pure V3 kernel — just with larger scratchpad
__global__ void xelis_stage3_bigpad(uint64_t *all_scratch, uint32_t batch_size)
{
...
int main(int argc, char **argv) {
    ...
    printf("Batch: %d hashes (%.1f GB)\n", batch, scratch_bytes/1024.0/1024.0/1024.0);
    ...
    // Warmup
    xelis_stage3_bigpad<<<grid, tpb>>>(d_scratch, batch);
    ...
    printf("PAD_MULT=%d, Scratchpad=%.1f MB, Batch=%d\n", PAD_MULT, pad_bytes/1024.0/1024.0, batch);
    printf("Warps/SM: %.1f\n", (float)batch/prop.multiProcessorCount/32.0);
    printf("%.2f kH/s (%.1f H/s per SM)\n", khs, khs*1000.0f/prop.multiProcessorCount);
    printf("\n");
