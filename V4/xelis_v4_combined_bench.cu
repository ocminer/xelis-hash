// xelis_v4_combined_bench.cu — S-box (occupancy limit) + TapeMix (divergence)
//
// S-box: 2KB/thread shared mem → 2 warps/SM on 5090, 1 warp on CMP
// TapeMix: per-hash instruction tape → warp divergence hits harder at low occupancy
// CPU: S-box in L1 (~free), tape branch-predicted (~free)
// Combined: both mechanisms reinforce each other
...
// ========================================================================
// COMBINED KERNEL: S-box (occupancy limit) + TapeMix (divergence penalty)
// ========================================================================
__global__ void __launch_bounds__(32, 2)
xelis_stage3_combined(uint64_t *all_scratch, uint32_t batch_size)
{
    ...
    for (uint32_t i = 0; i < SCRATCHPAD_ITERS; i++) {
        ...
        for (uint32_t j = 0; j < BUFFER_SIZE; j++) {
            ...
            // S-box read + write (occupancy limit, ~free on CPU via L1)
            uint64_t chain = a ^ sbox[(a >> 56) & 0xFF];
            state[((result >> 10) & (STATE_WORDS-1))] = chain;
            sbox[(chain >> 48) & 0xFF] ^= chain;

            // TapeMix (every TAPE_FREQ iters — divergence penalty on GPU)
            if ((j & (TAPE_FREQ - 1)) == 0) {
                #pragma unroll 1
                for (int t = 0; t < TAPE_LEN; t++) {
                    tape_state = tape_exec(tape[t], tape_state, result ^ a, c ^ b);
                }
                result ^= tape_state;
            }
...
// V3 baseline (same as other benches)
__global__ void xelis_stage3_v3(uint64_t *all_scratch, uint32_t batch_size)
{
    uint32_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= batch_size) return;
    uint64_t *a = all_scratch + (uint64_t)tid * MEMORY_SIZE;
    uint64_t *bb = a + BUFFER_SIZE;
    const uint8_t key[16] = {'x','e','l','i','s','h','a','s','h','-','p','o','w','-','v','3'};
    uint64_t aa = bb[BUFFER_SIZE-1], ab = a[BUFFER_SIZE-1] >> 32;
    uint32_t r = 0;
    for (uint32_t i = 0; i < SCRATCHPAD_ITERS; i++) {
        uint64_t ma = a[map_index(aa)], mb = bb[map_index(ma^ab)];
        uint8_t blk[16]; memcpy(blk,&mb,8); memcpy(blk+8,&ma,8);
        aes_single_round(blk,key);
        uint64_t h1; memcpy(&h1,blk,8); uint64_t h2; memcpy(&h2,blk+8,8);
        uint64_t res = ~(h1^h2);
        for (uint32_t j = 0; j < BUFFER_SIZE; j++) {
            uint64_t la=a[map_index(res)],lb=bb[map_index(la^~rotr64(res,r))];
            uint64_t c=(r<BUFFER_SIZE)?a[r]:bb[r-BUFFER_SIZE]; r=(r<MEMORY_SIZE-1)?r+1:0;
            uint32_t sel=rotl64(res,c)&0xf; uint64_t v;
            if(sel>=10)v=stage3_heavy128(sel,la,lb,c,res,r);
            else if(sel<=2)v=stage3_isqrt_heavy(sel,la,lb,c,res,i,j);
            else switch(sel){case 3:v=(la+lb)*c;break;case 4:v=(lb-c)*la;break;case 5:v=c-la+lb;break;
            case 6:v=la-lb+c;break;case 7:v=lb*c+la;break;case 8:v=c*la+lb;break;case 9:v=la*lb*c;break;default:v=0;}
            uint64_t is=v^res; res=rotl64(is,r);
            bool ub=pick_half(v); uint64_t it=map_index(is);
            uint64_t t=(ub?bb[it]:a[it])^res;
            uint64_t wa=map_index(t^res^0x9e3779b97f4a7c15ULL),wb=map_index(wa^~res^0xd2b74407b1ce6e93ULL);
            uint64_t oa=a[wa],ob=bb[wb]; bb[wb]=ob^(oa^rotr64(t,i+j)); a[wa]=t;
        }
        aa=modular_power(aa,ab,res); ab=isqrt_v3(res)*((uint64_t)r+1)*isqrt_v3(aa);
    }
    a[0]=aa^ab;
}

// S-box only (no tape) for comparison
__global__ void __launch_bounds__(32, 2)
xelis_stage3_sbox_only(uint64_t *all_scratch, uint32_t batch_size)
{
    extern __shared__ uint64_t shared_sbox2[];
    uint64_t *sbox = &shared_sbox2[threadIdx.x * SBOX_ENTRIES];
    uint32_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= batch_size) return;
    uint64_t *mem_buffer_a = all_scratch + (uint64_t)tid * MEMORY_SIZE;
    uint64_t *mem_buffer_b = mem_buffer_a + BUFFER_SIZE;
    ...
    #pragma unroll
    for (int k = 0; k < STATE_WORDS; k++) st[k] = mem_buffer_a[SBOX_ENTRIES+k];
    const uint8_t key[16] = {'x','e','l','i','s','h','a','s','h','-','p','o','w','-','v','4'};
    uint64_t addr_a = mem_buffer_b[BUFFER_SIZE-1], addr_b = mem_buffer_a[BUFFER_SIZE-1]>>32;
    uint32_t r = 0;
    for (uint32_t i = 0; i < SCRATCHPAD_ITERS; i++) {
        uint64_t mem_a=mem_buffer_a[map_index(addr_a)],mem_b=mem_buffer_b[map_index(mem_a^addr_b)];
        uint8_t block[16]; memcpy(block,&mem_b,8); memcpy(block+8,&mem_a,8);
        aes_single_round(block,key);
        uint64_t h1; memcpy(&h1,block,8); uint64_t h2; memcpy(&h2,block+8,8);
        uint64_t result=~(h1^h2);
        for (uint32_t j = 0; j < BUFFER_SIZE; j++) {
            uint64_t a=mem_buffer_a[map_index(result)],b=mem_buffer_b[map_index(a^~rotr64(result,r))];
            uint64_t c=(r<BUFFER_SIZE)?mem_buffer_a[r]:mem_buffer_b[r-BUFFER_SIZE]; r=(r<MEMORY_SIZE-1)?r+1:0;
            uint64_t chain=a^sbox[(a>>56)&0xFF];
            st[((result>>10)&(STATE_WORDS-1))]=chain;
            sbox[(chain>>48)&0xFF]^=chain;
            uint32_t sel=rotl64(result,c)&0xf; uint64_t v;
            if(sel>=10)v=stage3_heavy128(sel,a,b,c,result,r);
            else if(sel<=2)v=stage3_isqrt_heavy(sel,a,b,c,result,i,j);
            else switch(sel){case 3:v=(a+b)*c;break;case 4:v=(b-c)*a;break;case 5:v=c-a+b;break;
            case 6:v=a-b+c;break;case 7:v=b*c+a;break;case 8:v=c*a+b;break;case 9:v=a*b*c;break;default:v=0;}
            uint64_t idx_seed=(v^chain)^result; result=rotl64(idx_seed,r);
            bool use_b=pick_half(v); uint64_t idx_t=map_index(idx_seed);
            uint64_t t=(use_b?mem_buffer_b[idx_t]:mem_buffer_a[idx_t])^result;
            uint64_t w_a=map_index(t^result^0x9e3779b97f4a7c15ULL),w_b=map_index(w_a^~result^0xd2b74407b1ce6e93ULL);
            uint64_t old_a=mem_buffer_a[w_a],old_b=mem_buffer_b[w_b];
            mem_buffer_b[w_b]=old_b^(old_a^rotr64(t,i+j)); mem_buffer_a[w_a]=t;
        }
        addr_a=modular_power(addr_a,addr_b,result); addr_b=isqrt_v3(result)*((uint64_t)r+1)*isqrt_v3(addr_a);
    }
    mem_buffer_a[0]=addr_a^addr_b^st[0];
}
...
int main(int argc, char **argv) {
    ...
    printf("Combined S-box+TapeMix — TL=%d TF=%d\n", TAPE_LEN, TAPE_FREQ);
    printf("=============================================\n");
    ...
    // V3: full occupancy
    int v3_tpb=128, v3_grid=(batch+v3_tpb-1)/v3_tpb;
    // S-box kernels: 32 threads/block
    int sb_tpb=32, sb_grid=(batch+sb_tpb-1)/sb_tpb;
    size_t sb_smem = (size_t)sb_tpb * SBOX_ENTRIES * sizeof(uint64_t);

    CUDA_CHECK(cudaFuncSetAttribute(xelis_stage3_sbox_only,
               cudaFuncAttributeMaxDynamicSharedMemorySize, sb_smem));
    CUDA_CHECK(cudaFuncSetAttribute(xelis_stage3_combined,
               cudaFuncAttributeMaxDynamicSharedMemorySize, sb_smem));

    // Warmup all 3
    xelis_stage3_v3<<<v3_grid,v3_tpb>>>(d_scratch,batch);
    CUDA_CHECK(cudaDeviceSynchronize());
    xelis_stage3_sbox_only<<<sb_grid,sb_tpb,sb_smem>>>(d_scratch,batch);
    CUDA_CHECK(cudaDeviceSynchronize());
    xelis_stage3_combined<<<sb_grid,sb_tpb,sb_smem>>>(d_scratch,batch);
    CUDA_CHECK(cudaGetLastError());
    CUDA_CHECK(cudaDeviceSynchronize());

    auto bench = [&](const char* name, auto kernel, int grid, int tpb, size_t smem) {
        float avg = 0;
        for (int ri = 0; ri < runs; ri++) {
            CUDA_CHECK(cudaEventRecord(start));
            kernel<<<grid, tpb, smem>>>(d_scratch, batch);
        ...
        printf("%-12s: %.2f ms -> %7.2f kH/s (%5.1f H/s/SM)\n",
               name, avg, khs, khs*1000/prop.multiProcessorCount);
        return khs;
    };

    float v3  = bench("V3",       xelis_stage3_v3,       v3_grid, v3_tpb, 0);
    float sb  = bench("S-box",    xelis_stage3_sbox_only, sb_grid, sb_tpb, sb_smem);
    float cmb = bench("Combined", xelis_stage3_combined,  sb_grid, sb_tpb, sb_smem);

    printf("\n=============================================\n");
    printf("RESULTS: %s (%d SMs)\n", prop.name, prop.multiProcessorCount);
    printf("V3:       %7.2f kH/s (%5.1f H/s/SM)\n", v3, v3*1000/prop.multiProcessorCount);
    printf("S-box:    %7.2f kH/s (%5.1f H/s/SM) — %.2fx vs V3\n", sb, sb*1000/prop.multiProcessorCount, v3/sb);
    printf("Combined: %7.2f kH/s (%5.1f H/s/SM) — %.2fx vs V3\n", cmb, cmb*1000/prop.multiProcessorCount, v3/cmb);
    printf("Tape adds: %.1f%% overhead on top of S-box\n", (sb/cmb - 1)*100);
    printf("\n");
