// xelis_v4_hybrid_bench.cu — BigPad + AES mixing per iteration
//
// BigPad: VRAM limits occupancy (non-bypassable).
// AES mixing: CPU has AES-NI (1 cycle/round), GPU pays ~20 cycles/round.
// At 0.8 warps/SM from BigPad, GPU can't hide AES compute.
// CPU boost from hardware AES widens the gap further.
//
// Build: nvcc -O3 -arch=sm_120 -DPAD_MULT=6 -DAES_MIX=4 -o bench_hybrid ...
...
#endif
#ifndef AES_MIX
#define AES_MIX 0    // AES rounds per inner iteration
#endif
...
// BigPad + AES hybrid kernel
__global__ void xelis_stage3_hybrid(uint64_t *all_scratch, uint32_t batch_size)
{
...
#if AES_MIX > 0
            // AES mixing: CPU gets this for ~1 cycle/round via AES-NI.
            // GPU pays ~20 cycles/round in software. At <1 warp/SM, can't hide.
            {
                uint8_t aes_block[16];
                memcpy(aes_block, &a, 8);
                memcpy(aes_block+8, &b, 8);
                // Derive per-iteration round key from result
                uint8_t rkey[16];
                memcpy(rkey, &result, 8);
                memcpy(rkey+8, &c, 8);
                #pragma unroll
                for (int aes_r = 0; aes_r < AES_MIX; aes_r++) {
                    aes_single_round(aes_block, rkey);
                }
                uint64_t aes_out;
                memcpy(&aes_out, aes_block, 8);
                // Feed AES output into the branch operation inputs
                a ^= aes_out;
                b ^= aes_out >> 32;
            }
#endif
...
int main(int argc, char **argv) {
    ...
    printf("Hybrid Benchmark — PAD_MULT=%d, AES_MIX=%d\n", PAD_MULT, AES_MIX);
    printf("================================================\n");
    printf("Device %d: %s (%d SMs, %.1f GB)\n", dev, prop.name,
           prop.multiProcessorCount, prop.totalGlobalMem/(1024.0*1024.0*1024.0));
    printf("Scratchpad: %.1f MB, Inner iters: %u\n", pad_bytes/1024.0/1024.0, SCRATCHPAD_ITERS*BUFFER_SIZE);
    ...
    // Warmup
    xelis_stage3_hybrid<<<grid,tpb>>>(d_scratch,batch);
    ...
    for (int r_idx = 0; r_idx < runs; r_idx++) {
        ...
        printf("  Run %d: %.2f ms (%.2f kH/s)\n", r_idx+1, ms, batch/(ms/1000.0f)/1000.0f);
        if (r_idx > 0) avg += ms;
    }
    avg /= (runs > 1 ? runs-1 : 1);
    float khs = batch / (avg/1000.0f) / 1000.0f;

    printf("\nRESULTS: PAD=%d AES=%d | %s %d SMs | %.2f kH/s (%.1f H/s/SM) | W/SM=%.1f\n\n",
           PAD_MULT, AES_MIX, prop.name, prop.multiProcessorCount,
           khs, khs*1000.0f/prop.multiProcessorCount,
           (float)batch/prop.multiProcessorCount/32.0);
